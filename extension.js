...
const UpyunUpLoad = require('./lib/upload');

exports.activate = (context) => {
    const disposable = vscode.commands.registerCommand('extension.pasteImageToUpYun', () => {
        start();
    });
...
function start() {
    ...
    let config = vscode.workspace.getConfiguration('pasteImageToUpYun');
    ...
    createImageDirWithImagePath(imagePath).then(imagePath => {
        saveClipboardImageToFileAndGetPath(imagePath, (imagePath) => {
            ...
            UpyunUpLoad(config, imagePath, mdFilePath).then(({ name, url }) => {
